package com.lirui.demo.structure;
...
public class AvlTreeDemo {
    ...
    public static class AVLTree{
        ...
        public static AvlNode query(int data, AvlNode node) {
            if (node == null) {
                return null;
            }
            if (node.data == data) {
                ...
                return query(data, node.lchild);
            } else {
                return query(data, node.rchild);
            }
        }
    }
    ...
    public static void main(String[] args) {
        AvlNode node = null;
        int[] data = {1,4,5,6,7,10,9,8};
        for (int i : data) {
            node = AVLTree.insert(i, node);
        }

        AvlNode result = AVLTree.query(6, node);
    }
}
